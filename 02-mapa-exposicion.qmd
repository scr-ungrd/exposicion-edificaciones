...
> **Nota metodológica:** Las huellas de edificaciones provienen del conjunto de datos [Open Buildings](https://sites.research.google/open-buildings/) de Google, obtenidas vía Google Earth Engine y convertidas al formato [PMTiles](https://docs.protomaps.com) con [tippecanoe](https://github.com/felt/tippecanoe) para distribución eficiente vía HTTP Range Requests. La cobertura actual corresponde al corredor andino suroccidental de Colombia (no a la totalidad del territorio nacional).

<!-- TODO: data/edificios.pmtiles (410 MB) no está en git (supera el límite de 100 MB de GitHub
     y GitHub Pages no sirve archivos LFS). Para que este mapa funcione en el sitio publicado,
     hay que subir el archivo a un host externo con Range Requests + CORS (p.ej. Google Cloud
     Storage) y apuntar `pmtilesUrl` allá. Localmente solo funciona con `npx serve` (no con
     `quarto preview`, que no soporta Range Requests). Ver data/generar_pmtiles.sh para regenerarlo. -->
